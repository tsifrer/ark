

def is_block_chained(app, previous_block, next_block):
    follows_previous = next_block.previous_block == previous_block.id

    print(previous_block.id)
    print(next_block.previous_block)
    is_plus_one = next_block.height == previous_block.height + 1

    previous_slot = app.slots.get_slot_number(previous_block.height, previous_block.timestamp)
    next_slot = app.slots.get_slot_number(next_block.height, next_block.timestamp)
    is_after_previous_slot = previous_slot < next_slot

    print('follows_previous', follows_previous)
    print('is_plus_one', is_plus_one)
    print('is_after_previous_slot', is_after_previous_slot)
    return follows_previous and is_plus_one and is_after_previous_slot


def is_block_exception(app, block):
    exception_blocks = app.config['exceptions'].get('blocks', [])
    return block.id in exception_blocks
